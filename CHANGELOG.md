...
- [#4332](https://github.com/pulumi/pulumi-kubernetes/issues/4332) Implement the `List` provider RPC and advertise `listInputs` (`namespace`, `name`, `labelSelector`, `fieldSelector`) on every non-nested resource. `namespace` is omitted from `listInputs` on cluster-scoped kinds. See [`docs/list-rpc-semantics.md`](docs/list-rpc-semantics.md) for design notes.
...
- [#2926](https://github.com/pulumi/pulumi-kubernetes/issues/2926) Use client-side create for new resources instead of server-side apply.
...
- [#4181](https://github.com/pulumi/pulumi-kubernetes/pull/4181) Fix PVC timeout when updating to a storage class with WaitForFirstConsumer bind mode.

### Added

- [#3661](https://github.com/pulumi/pulumi-kubernetes/pull/3661) Support for OCI login inside the provider process. Providing a username/password to `Chart v4` and `Release v3` will now login to private OCI
